 ...
 */
class LaborsModelLabors extends FarmAppModelFarmApp
{
	...
	public function _getListQuery()
	{
		...
		// Select some fields
		$query->select('b.id,b.name,b.description,b.status,b.unit_cost');
		// From the labor_category table
		$query->from('#__labor_category as b');
		$query->order($db->getEscaped($this->getState('list.ordering')).' '.$db->getEscaped($this->getState('list.direction')));
		if (!empty($search)) {
			$query->where('(b.name LIKE '.$db->Quote('%'.$db->getEscaped($search, true).'%').')');
		}
		return $query;
	}
...
/*
	 * function is for doing the publish/unpublish the labor category in default.php of labors templates..
	 * 
	 */
...
/**
	 ...
	 */
		function delete()
		{
		    ...
		    $row =& $this->getTable('Labor','LaborTable');
		  //  var_dump($cids); die;
		    foreach($cids as $cid) {
		  		if (!$row->delete( $cid )) {
...
/**
	 ...
	 */
		function multiplepublish()
		{
		    ...
		    foreach($cids as $cid) {
			 	$query = 'UPDATE #__labor_category SET status = 1'.
                		 '  WHERE id = '.$cid;  
                $db->setQuery($query);
                $db->query();
		     }
	 ...
	 */
		function multipleunpublish()
		{
		    ...
		    foreach($cids as $cid) {
				$query = 'UPDATE #__labor_category SET status = 0'.
                		 '  WHERE id = '.$cid; 
                $db->setQuery($query);
                $db->query();
		     }
