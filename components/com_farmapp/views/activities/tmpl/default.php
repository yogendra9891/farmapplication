...
if($this->escape($this->state->get('filter.search_date')))
{
 $searchhdate = $this->escape($this->state->get('filter.search_date'));
}
else
{
	$searchhdate = '';
...
<form action="<?php echo JRoute::_('index.php?option=com_farmapp&view=activities'); ?>" method="post" name="adminForm" id="adminForm">
	   <h2 class="titleheadingfrontend">Calendar of Activities</h2>
        ...
        <div><span class="viewofactivity">VIEW :&nbsp;</span> <a href="<?php echo JRoute::_('index.php?option=com_farmapp&view=activities'); ?>" class="activemodeofactivity"> Daily </a>&nbsp; |
           &nbsp;  <a href="<?php echo JRoute::_('index.php?option=com_farmapp&view=activities&task=weeklyactivities'); ?>">  Weekly </a>&nbsp;  |  
            &nbsp; <a href="<?php echo JRoute::_('index.php?option=com_farmapp&view=activities&task=monthlyactivities'); ?>"> Monthly </a></div>
        <div class="dailyactivitycalendar">
         <?php echo $this->actdate->getInput('dateofactivity','',$searchhdate); ?>
         <button type="submit"><?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?></button></div>
	...
	<div class="filter-search fltlft">
		...
		<div class="filter-select fltrt">
			...
			<select name="filter_zone_id" class="inputbox" onchange="this.form.submit()">
				<?php echo JHtml::_('select.options', FarmappHelper::getZoneBedOptions(), 'value', 'text', $this->state->get('filter.zone_id'));?>
			</select>
...
(function($){
	 $(document).ready(function(){
		 $('dateofactivity').val(<?php echo $this->escape($this->state->get('filter.search_date')); ?>);
	 });
});	 


window.addEvent('domready', function() { 
    $('dateofactivity').addEvent('onpropertychange', function(e) {
////    //	$('bedid').empty();
    	e.stop(); 
alert('gfgf');
       });

//	#dateofactivity.live('change', function(event) { }); 
    });
</script>
