...
JLoader::import( 'com_farmapp.models.fields.beds', JPATH_ROOT.DS.'components' );
JFormHelper::loadFieldClass('list');
/**
 * Methods supporting a list of Crops records.
 */
class CropsModelCrops extends FarmAppModelFarmApp
{
	...
	public function _getListQuery()
	{
		...
		$bedid = $this->getState('filter.bed_id');
		...
		$query->join('INNER', $db->quoteName('#__farm').' AS f ON c.farm = f.id');
		...
		if($bedid && $bedid!=0) {
			$query->where(' c.bed_id = '.(int) $bedid);
		}
		if($zoneid && $zoneid!=0) {
...
/**
	 ...
	 */
	function getbedslist($zoneid,$cropid)
	{    
	    $zoneid = (int)$zoneid;
	    $cropid = $cropid;
	    $options = array();
		...
		$query3->select('f.id');
		$query3->from('#__farm as f');
		$query3->where('f.userid = '.$userid);
		$db->setQuery($query3);
        ...
        echo '<select id="bed_id" class="required list" name="bed_id[]" multiple="multiple" size="6" required="required">';
